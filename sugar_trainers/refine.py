...
def refined_training(args):
    ...
    with Progress(
        ...
        task = progress.add_task("Refining", total=num_iterations, loss=0.0)
        ...
        for batch in (range(9_999_999)):
            ...
            for camera_indices, gt_image in enumerate(nerfmodel.training_cameras.image_dataloader):
                ...
                progress.update(task, loss=loss.detach().item())

                if (iteration > 0) and (current_sh_levels < sh_levels) and (iteration % 1000 == 0):
                    current_sh_levels += 1
